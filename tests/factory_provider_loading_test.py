...
Cherry-picked from upstream PRs #349 and #351 (Fixes #335).
Ensures that providers.load_builtins_once() is called before
resolving providers, even when provider is explicitly set via
ModelConfig.
"""
...
class FactoryProviderLoadingTest(absltest.TestCase):
    ...
    def test_create_model_with_schema_loads_builtins_explicit_provider(self):
        ...
        try:
            ...
            # If the bug exists, the error message starts with
            # "No provider found matching"
            if "No provider found matching" in str(e):
                self.fail(f"Bug reproduced: Builtin provider not loaded! Error: {e}")

            # If we get "API key not provided" or similar, it means
            # the class WAS found! This is SUCCESS.
        except ImportError:
            # Also success - means we tried to import dependencies
            # (so we found the class)
            pass
